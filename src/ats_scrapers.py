...
def scrape_workday(company_name: str, careers_url: str) -> list[dict[str, Any]]:
    ...
    fails (many companies customize this), we return an empty list —
    this company just needs its careers_url corrected by hand in
    companies.yaml (see SETUP_GUIDE.md).
    """
    m = re.match(
...
def scrape_generic(company_name: str, careers_url: str) -> list[dict[str, Any]]:
    """
    Best-effort fallback for companies not on a known ATS platform, or
    where the specific board token/tenant isn't known. Looks for links
    on the careers page whose visible text looks like a job title.

    This will not be as reliable as the API-based scrapers above. If a
    company consistently returns nothing useful here, the fix is almost
    always: find their actual ATS (see SETUP_GUIDE.md, "Adding or fixing
    a company") and set ats: to the right platform in companies.yaml.
    """
    ...
    for a in soup.find_all("a", href=True):
        ...
        if href in seen_urls or not href.startswith("http"):
            continue
        seen_urls.add(href)
        jobs.append({"title": text, "location": "", "url": href, "description": ""})
    return jobs
...
def scrape_company(company: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    try:
        if ats == "greenhouse":
            return scrape_greenhouse(name, token)
        if ats == "lever":
            return scrape_lever(name, token)
        if ats == "smartrecruiters":
            return scrape_smartrecruiters(name, token)
        if ats == "personio":
            return scrape_personio(name, token)
        if ats == "workday":
            return scrape_workday(name, careers_url)
        if ats == "auto":
